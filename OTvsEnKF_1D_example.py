...
LearningRate = 1e-2
ITERATION = int(1024) 
Final_Number_ITERATION = int(32*2)
#N_test = int(1e3)
Time_step = N 
...
for j in range(AVG_SIM):
    ...
    plt.subplot(2,1,1)
    for i in range(J):
        plt.plot(t,SAVE_X_EnKF[j,:,:,i],alpha = 0.2)
    plt.plot(t,SAVE_True_X[j, ],'k--',label = 'dns')
    plt.xlabel('time')
    ...
    for i in range(SAMPLE_SIZE):
        plt.plot(t,SAVE_all_X_OT[j,:,:,i],alpha = 0.5)
    plt.plot(t,SAVE_True_X[j, ],'k--',label = 'dns')
    plt.xlabel('time')
    plt.ylabel('OT state')
    plt.legend()
...
#%%
# =============================================================================
# np.savez('3.12_DATA_File_Simulations_Linaer_abs_X_Normalized_2_BlocksResidual_100_simulations.npz',\
#     time = t, Y_true = SAVE_True_Y,X_true = SAVE_True_X,\
#     X_EnKF = SAVE_X_EnKF , X_OT = SAVE_all_X_OT ,  MSE_EnKF = MSE_EnKF , MSE_OT=MSE_OT)
# =============================================================================
