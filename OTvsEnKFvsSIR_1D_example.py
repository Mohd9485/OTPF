...
for k in range(AVG_SIM):
    ...
    for i in range(N-1):
        ...
        weight = np.exp(-(SAVE_True_Y[k,i+1,] - h(x_SIR[k,i+1,]))*(SAVE_True_Y[k,i+1] - h(x_SIR[k,i+1,]))/(2*gamma*gamma))
        W[k,i+1,] = weight/np.sum(weight,axis=1)
...
# 
#     for i in range(J):
#         plt.plot(t,x_SIR[j,:,:,i],alpha = 0.5)
#     plt.plot(t,SAVE_True_X[j, ],'k--',label = 'dns')
#     plt.xlabel('time')
